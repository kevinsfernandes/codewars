...
function longest_consec(strarr, k) {
    if (k < 1 || k >strarr.length || strarr.length == 0){
        return ""
    }
    let sArr = strarr.map ( (el, i, arr) => arr.slice(i, i+k).join(''))
    return sArr.reduce((longest, current) => longest.length >= current.length ? longest : current)
  }
